    ...
    if request.method == 'POST':
        ...
        if int(realizado.getvalue()) == 1:
            ...
            usuario = Usuario.objects.create_user(
                rut, id_tipo_empleado, email=email)
...
def listarusuario(request):
    ...
    if request.method == 'POST':
        ...
        if rut_tutor == rut_paciente:
            messages.success(
                request, "El Rut del tutor y del usuario no pueden ser el mismo.")
        else:
            # Conexión a la bd
            bd = ConexionBD()
            con = bd.conectar()
            ...
            cursor.callfunc('pkg_crear_usuario.fn_existe_persona',
                            existe_tutor_tabla_persona, [rut_tutor])
            if existe_tutor_tabla_persona == False:
                messages.success(
                    request, "<<¡No existe registro del rut del tutor en nuestro sistema, por favor ingresa sus datos en el apartado Crear Usuario.")
            else:
...
def cambiarCentroSalud(request):
    ...
    if request.method == 'POST':
        ...
        cursor.callproc('sp_cambio_centro_salud', [
                        rut_persona, request.user.rut.id_centro.id_centro, resultado])
...
def inicioFarmacia(request):
    ...
    respuesta = cursor.callfunc(
        'pkg_farmacia.fn_existe_mail_no_enviado', bool, [])
    if respuesta:
        query = (
        ...
        for fila in filas:
            ...
            msg = 'Sr(a) ' + paciente + ' (' + rut_paciente + ') ' ' la reserva de medicamento N° ' + \
                str(id_reserva) + ' se encuentra disponible para su retiro.'
            ...
            cursor.callproc(
                'pkg_farmacia.sp_marcar_mail_enviado', [id_reserva])
...
def entregaMedicamento(request, id_receta):
    ...
    if request.method == 'POST':
        ...
        if opcForm == 1:
            ...
            if existeStock:
                ...
                cursor.callproc('pkg_farmacia.sp_entregar_medicamento', [
                                cantidadEntrega, codMed, id_receta, request.user.rut.rut, tipoEntrega, resultado])
        ...
        if opcForm == 2:
            ...
            cursor = con.cursor()
            resultado = cursor.var(int)
            cursor.callproc('pkg_farmacia.sp_crear_reserva_medicamento', [
                            codMed, id_receta, request.user.rut.rut, cantidadReserva, resultado])
            if resultado.getvalue() == 1:
...
def entregaResultado(request, id_receta, codigo_med, cantidad, numMensaje):
    ...
    if numMensaje == 1:
        ...
        medicamento = Medicamento.objects.get(codigo=codigo_med)
        mensaje = 'Reserva del Medicamento ' + \
            medicamento.nombre_medicamento + ' realizada.'
        datos = {
            'numero_mensaje': numMensaje,
            'mensaje': mensaje,
            'cantidad': cantidad,
        ...
        reserva = ReservaMedicamento.objects.filter(
            id_receta=id_receta).filter(codigo=codigo_med)
        ...
        medicamento = Medicamento.objects.get(codigo=codigo_med)
        mensaje = 'El Medicamento ' + medicamento.nombre_medicamento + \
            ' ha sido marcado para Adquirir Particularmente.'
        datos = {
            'numero_mensaje': numMensaje,
            'mensaje': mensaje,
            'medicamento': medicamento,
...
def reservaDetalle(request, id_reserva):
    ...
    if request.method == 'POST':
        ...
        cursor = con.cursor()
        resultado = cursor.var(int)
        cursor.callproc('pkg_farmacia.sp_entregar_medicamento_reserva', [
                        spIdReserva, request.user.rut.rut, resultado])
        if resultado.getvalue() == 1:
...
def agregarpaciente(request):
    ...
    regiones = Region.objects.filter(
        id_region=request.user.rut.id_comuna.id_region.id_region)
    # ciudades = Comuna.objects.all().order_by('nombre_comuna')
    ciudades = Comuna.objects.filter(
        id_region=request.user.rut.id_comuna.id_region).order_by("nombre_comuna")
    # centro_salud = CentroSalud.objects.all().order_by('id_comuna')
    centro_salud = CentroSalud.objects.filter(
        id_centro=request.user.rut.id_centro.id_centro)
        ...
        if campoBuscar is not None:
            ...
            for i in persona:
                if i.rut == campoBuscar:
                    ...
                    messages.success(
                        request, 'Tutor encontrado en el sistema.')
    elif request.method == 'POST':
